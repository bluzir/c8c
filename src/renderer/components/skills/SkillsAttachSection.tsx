    ...
    <section className="space-y-3" aria-busy={loading}>
      ...
      {loading ? (
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_22rem]">
          <div className="space-y-4">
            {Array.from({ length: 2 }).map((_, sectionIndex) => (
              <section
                key={`skills-attach-skeleton-section-${sectionIndex}`}
                className="overflow-hidden rounded-lg surface-panel"
              >
                <div className="border-b border-hairline flex items-center justify-between gap-3 px-4 py-3">
                ...
                <div className="divide-y divide-hairline">
                  {Array.from({ length: 4 }).map((__, rowIndex) => (
                    <div
                      key={`skills-attach-skeleton-row-${sectionIndex}-${rowIndex}`}
          ...
          <div className="hidden lg:block ui-slab">
        ...
        <div className="ui-empty-state-box ui-empty-state px-4">
          No skills match this filter. Install a library or plugin, or clear
          search.
        </div>
      ) : (
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_22rem]">
          <div className="min-w-0 flex-1 space-y-4">
            {groupedSkills.map((section) => (
              <section
                key={section.id}
                className="rounded-lg surface-panel overflow-hidden"
              >
                <div className="border-b border-hairline flex items-center justify-between gap-3 px-4 py-3">
                  <div>
