...
export function MultiRunDashboardDetail({
  ...
  now: number
  onFocusWorkflow: (entry: DashboardEntry) => Promise<void>
  ...
  return (
    <div key={entry.workflowKey} className="flex h-full flex-col ui-fade-slide-in">
      ...
      <div className="ui-scroll-region flex-1 overflow-y-auto px-5 py-4">
        <section className="rounded-xl surface-panel p-5 space-y-5 ui-fade-slide-in">
          <div className="space-y-2">
            <p className="section-kicker">{groupMetaLabel(triageGroup(entry))}</p>
            <h4 className="text-title-md text-foreground">{triageHeadline(entry)}</h4>
            <p className="text-body-sm text-muted-foreground">{triageSummary(entry, now)}</p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 border-t border-hairline pt-4">
            {entry.runStatus === "paused" && entry.runId ? (
              ...
              <Button size="sm" onClick={() => void onFocusWorkflow(entry)}>
                <Eye size={14} />
                {primaryActionLabel(entry)}
              </Button>
            )}
            ...
            {entry.runStatus === "paused" && (
              <Button variant="outline" size="sm" onClick={() => void onFocusWorkflow(entry)}>
                <ExternalLink size={14} />
                Open flow
              </Button>
            )}

            {isRunInFlight(entry.runStatus) && entry.runStatus !== "paused" && entry.runStatus !== "cancelling" && entry.runStatus !== "starting" && entry.runId ? (
              <Button variant="outline" size="sm" onClick={() => void onPauseExecution(entry)}>
                <Pause size={14} />
                Pause
              </Button>
            ) : null}
            ...
            {entry.runStatus === "cancelling" ? (
              ...
              <Button variant="destructive" size="sm" onClick={() => void onCancelExecution(entry)}>
                <Square size={14} />
                Stop
              </Button>
            ) : (
              <Button variant="ghost" size="sm" onClick={() => onClearEntry(entry)}>
                <TimerReset size={14} />
                Clear
              </Button>
            )}
          ...
          <div className="grid grid-cols-1 gap-x-6 gap-y-3 border-t border-hairline pt-4 text-body-sm md:grid-cols-2">
            <div className="flex items-start justify-between gap-3">
              ...
              <span className="text-right text-foreground">{folderName(entry.projectPath)}</span>
            </div>
            <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex items-start justify-between gap-3">
              ...
              <span className="text-right text-foreground">
                {entry.approvalCount > 0 ? `${entry.approvalCount} pending` : "None"}
              </span>
            </div>
            <div className="flex items-start justify-between gap-3">
              <span className="text-muted-foreground">Started</span>
              <span className="text-right text-foreground">{formatDateTime(entry.runStartedAt)}</span>
            </div>
            <div className="flex items-start justify-between gap-3">
          ...
          <div className="space-y-3 border-t border-hairline pt-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {entry.workspace ? (
                  <Button variant="ghost" size="sm" onClick={() => void window.api.showInFinder(entry.workspace!)}>
