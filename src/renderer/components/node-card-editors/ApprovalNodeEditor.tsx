...
export function ApprovalNodeEditor({
  ...
  return (
    <div className={EDITOR_PANEL_CLASS}>
      <p className="ui-meta-text text-muted-foreground">Pauses the flow and asks you to review before continuing.</p>
      ...
      <div>
        ...
        <TextareaWithMention
          ...
          onChange={(event) => onConfigChange({ ...config, message: event.target.value })}
          rows={3}
        ...
        {config.timeout_minutes != null && config.timeout_minutes > 0 && (
          <div className="flex items-center justify-between gap-2">
            ...
            <Select
              ...
              onValueChange={(value) =>
                onConfigChange({ ...config, timeout_action: value as ApprovalNodeConfig["timeout_action"] })
              }
            >
      ...
      <p className="ui-meta-text text-muted-foreground">Rejecting at this approval will stop the entire flow run.</p>
