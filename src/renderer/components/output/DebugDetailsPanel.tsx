...
export function DebugDetailsPanel({
  ...
  const toolCallSummaries = useMemo(
    () => buildToolCallSummaries(rawLog),
    [rawLog],
  )
  const toolCallCountLabel = useMemo(
    () => buildToolCallCountLabel(toolCallSummaries),
    [toolCallSummaries],
  )
  ...
  const maxTurns =
    nodeConfig && "maxTurns" in nodeConfig
      ? (nodeConfig as { maxTurns?: number }).maxTurns
      : undefined
  const evalThreshold =
    nodeType === "evaluator" && nodeConfig && "threshold" in nodeConfig
      ? (nodeConfig as { threshold: number }).threshold
      : undefined
  const evalMaxRetries =
    nodeType === "evaluator" && nodeConfig && "maxRetries" in nodeConfig
      ? (nodeConfig as { maxRetries: number }).maxRetries
      : undefined
  const evalRetryFrom =
    nodeType === "evaluator" && nodeConfig && "retryFrom" in nodeConfig
      ? (nodeConfig as { retryFrom?: string }).retryFrom
      : undefined

  const hasExecutionSummary = Boolean(
    durationMs ||
    state.metrics ||
    state.meta?.model_id ||
    maxTurns ||
  ...
  if (!hasExecutionSummary && !hasToolCalls && !hasEvalDetails && !hasRawLog)
    return null
  ...
  return (
    <DisclosurePanel
      ...
      surface="plain"
      className="ui-inset-well"
      summaryClassName="px-0 py-0"
      contentClassName="space-y-3 px-0 pt-3 pb-0"
    >
      {hasExecutionSummary && (
        <div className="space-y-1">
          ...
          <div className="space-y-1 border-l-2 border-hairline pl-3">
            {durationMs != null && (
            ...
            {state.metrics && (
              <>
                <div className="flex justify-between ui-meta-text">
                  ...
                  <span className="font-mono text-foreground">
                    {formatTokens(state.metrics.tokens_in)}
                  </span>
                </div>
                <div className="flex justify-between ui-meta-text">
                  <span className="text-muted-foreground">Tokens out</span>
                  <span className="font-mono text-foreground">
                    {formatTokens(state.metrics.tokens_out)}
                  </span>
                </div>
      ...
      {hasToolCalls && (
        <div className="space-y-1">
          ...
          <div className="space-y-1 border-l-2 border-hairline pl-3 max-h-48 overflow-y-auto ui-scroll-region">
          ...
          <div className="space-y-1 border-l-2 border-hairline pl-3">
            ...
            {evalResults.length > 0 && (
              <div className="flex justify-between ui-meta-text">
                ...
                <span className="font-mono text-foreground">
                  {evalResults.length}
                  {evalMaxRetries != null ? ` / ${evalMaxRetries + 1}` : ""}
                </span>
              </div>
            )}
            {evalResults.length > 0 && (
              <div className="border-t border-hairline mt-1.5 pt-1.5 space-y-1">
                {evalResults.map((evalResult) => (
                  <div key={evalResult.attempt} className="space-y-1">
                    ...
                    >
                      Attempt {evalResult.attempt}: {evalResult.score}/10{" "}
      ...
      {hasRawLog && (
        <div className="space-y-1">
          ...
          {rawLogOpen && (
            <pre className="ui-inset-well px-3 py-2 ui-meta-text text-muted-foreground whitespace-pre-wrap font-mono max-h-80 overflow-y-auto ui-scroll-region">
              {rawLogText}
            </pre>
          )}
        </div>
