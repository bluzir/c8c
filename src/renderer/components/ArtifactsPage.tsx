...
export function ArtifactsPage() {
  ...
  const [artifactPreviewById, setArtifactPreviewById] = useState<
    Record<
      string,
      {
        content: string
  ...
  const artifactFactoryKey = useCallback(
    (artifact: ArtifactRecord) => {
  ...
  const factoryScopeArtifacts = useMemo(
    () =>
      selectedFactoryId
        ? artifacts.filter(
            (artifact) => artifactFactoryKey(artifact) === selectedFactoryId,
          )
        : artifacts,
  ...
  const selectedFactoryLabel = useMemo(() => {
    ...
    const direct = factoryScopeArtifacts.find(
      (artifact) => artifact.factoryLabel,
    )?.factoryLabel
    if (direct) return direct
    if (selectedFactoryId.startsWith("pack:")) {
      const packId = selectedFactoryId.replace(/^pack:/, "")
      return (
        templates.find((template) => template.pack?.id === packId)?.pack
          ?.label || "Lab"
      )
    }
    return "Lab"
  ...
  const artifactKinds = useMemo(() => {
    return Array.from(
      new Set(factoryScopeArtifacts.map((artifact) => artifact.kind)),
    ).sort((left, right) =>
      formatArtifactContractLabel(left).localeCompare(
        formatArtifactContractLabel(right),
      ),
    )
  ...
  const caseOptions = useMemo(() => {
    return Array.from(artifactsByCaseKey.entries())
      .map(([id, caseArtifacts]) => {
        const latestArtifact = [...caseArtifacts].sort(
  ...
  const scopeArtifacts = useMemo(
    () =>
      selectedCaseId
        ? artifactsByCaseKey.get(selectedCaseId) || []
        : factoryScopeArtifacts,
  ...
  const compatibleTemplates = useMemo(() => {
    return templates
      ...
      .filter((template) =>
        areTemplateContractsSatisfied(template.contractIn, scopeArtifacts),
      )
  ...
  const matchingTemplatesByArtifactId = useMemo(() => {
    ...
    for (const artifact of factoryScopeArtifacts) {
      ...
      const matchingTemplates = compatibleTemplates
        .filter((template) =>
          areTemplateContractsSatisfied(template.contractIn, artifactScope),
        )
        .filter((template) =>
          template.contractIn?.some((contract) =>
            artifactScope.some((candidate) => candidate.kind === contract.kind),
          ),
        )
        .slice(0, 3)
  ...
  const selectedArtifactScope = useMemo(() => {
    ...
    return (
      artifactsByCaseKey.get(deriveArtifactCaseKey(selectedArtifact)) || [
        selectedArtifact,
      ]
    )
  ...
  const selectedArtifactMatchingTemplates = useMemo(
    () =>
      selectedArtifact
        ? matchingTemplatesByArtifactId.get(selectedArtifact.id) || []
        : [],
  ...
  useEffect(() => {
    ...
    void window.api
      ...
      .finally(() => {
        if (artifactPreviewRequestIdRef.current !== requestId) return
        setArtifactPreviewLoadingId((current) =>
          current === selectedArtifact.id ? null : current,
        )
      })
    ...
    return (
      <div className={className}>
        <ArtifactInspectPanel
          ...
          loading={
            artifactPreviewLoadingId === selectedArtifact.id &&
  ...
  const renderSelectionPrompt = (className?: string) => (
    <div className={className}>
      <section className="surface-figure px-4 py-5">
        <div className="space-y-2">
          <p className="ui-meta-label text-muted-foreground">Inspect results</p>
          <h2 className="text-title-sm font-semibold text-foreground">
            Select a saved result to inspect it
          </h2>
          <p className="text-body-sm text-muted-foreground">
            Review provenance, preview the saved output, and launch the next
            step from one owner surface instead of acting from the list.
          </p>
  ...
  const launchTemplate = async (
    ...
    sourceArtifacts = scopeArtifacts,
  ) => {
    ...
    try {
      const launch = await prepareTemplateStageLaunch({
        ...
        factory:
          selectedFactoryId && selectedFactoryLabel
            ? {
  ...
  return (
    <PageShell>
      <PageHeader
        title="Results"
        subtitle={`Browse saved results for ${projectFolderName(selectedProject)} and open the next step without rebuilding context.`}
        actions={
          <>
            {factoryBetaEnabled ? (
              <Button
              ...
              <>
                ...
                {caseOptions.map((entry) => (
                  <Button
                    key={entry.id}
                    variant={
                      selectedCaseId === entry.id ? "secondary" : "outline"
                    }
                    size="xs"
      ...
      <section
        className="space-y-4"
        aria-busy={artifactsLoading || templatesLoading}
      >
        ...
        {artifactsError ? (
          ...
          <div className="ui-empty-state ui-empty-state-box px-4">
            <Loader2 size={16} className="animate-spin text-muted-foreground" />
            <span>Loading project results and next steps...</span>
          </div>
          ...
          <>
            ...
            <div
              className={cn(
                ...
                selectedArtifact && "xl:grid-cols-[minmax(0,1fr)_28rem]",
              )}
            >
              <div className="min-w-0">
                <div className="ui-slab space-y-0">
                  {filteredArtifacts.map((artifact) => {
                    ...
                    const artifactCase =
                      caseOptions.find(
                        (entry) => entry.id === artifactCaseKey,
                      ) || null
                    const artifactScope = selectedCaseId
                      ? scopeArtifacts
                ...
                {selectedArtifact ? renderInspectPanel("mt-4 xl:hidden") : null}
              </div>
