...
function resolveNodeKind(
  ...
  const node =
    workflow.nodes.find((candidate) => candidate.id === templateNodeId) ||
    runtimeNodes.find((candidate) => candidate.id === templateNodeId) ||
    workflow.nodes.find((candidate) => candidate.id === nodeId) ||
    runtimeNodes.find((candidate) => candidate.id === nodeId)
  return node
    ? getRuntimeStagePresentation(node, { fallbackId: node.id }).kind
    : null
}
...
export function buildRunProgressSummary({
  ...
  const topLevelStepNodes = workflow.nodes.filter((node) =>
    isStepNodeType(node.type),
  )
  ...
  const splitterNodes = topLevelStepNodes.filter(
  ...
  const activeStageId = activeNodeId
    ? runtimeMeta[activeNodeId]?.templateId ||
      runtimeMeta[activeNodeId]?.splitterId ||
      activeNodeId
    : null
  const fallbackActiveStageId =
    activeStageId ||
    topLevelStepNodes.find((node) => getStageStatus(node) === "waiting_human")
      ?.id ||
    topLevelStepNodes.find(
      (node) => getStageStatus(node) === "waiting_approval",
    )?.id ||
  ...
  const activeStageNode = fallbackActiveStageId
    ? topLevelStepNodes.find((node) => node.id === fallbackActiveStageId) ||
      null
    : null
  const activeBranchSummary = activeStageNode
    ? getStageBranchSummary(activeStageNode)
    : null
