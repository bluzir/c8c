...
const api: C8cApi = {
  ...
  removeProject: (path: string) => invokeIpc<C8cApi["removeProject"]>("projects:remove", path),
  ...
  getSelectedProject: () => invokeIpc<C8cApi["getSelectedProject"]>("projects:get-selected"),

  // Skills
  scanSkills: (projectPath: string) => invokeIpc<C8cApi["scanSkills"]>("skills:scan", projectPath),
  createSkillTemplate: (projectPath: string) =>
    invokeIpc<C8cApi["createSkillTemplate"]>("skills:create-template", projectPath),
  scaffoldMissingSkills: (
    workflow: Workflow,
    availableSkills: Pick<DiscoveredSkill, "name" | "category">[],
    projectPath: string,
  ) => invokeIpc<C8cApi["scaffoldMissingSkills"]>("skills:scaffold", workflow, availableSkills, projectPath),
  ...
  listProjectWorkflows: (projectPath: string) =>
    invokeIpc<C8cApi["listProjectWorkflows"]>("workflows:list-project", projectPath),
  listGlobalWorkflows: () => invokeIpc<C8cApi["listGlobalWorkflows"]>("workflows:list-global"),
  loadWorkflow: (filePath: string) => invokeIpc<C8cApi["loadWorkflow"]>("workflows:load", filePath),
  ...
  saveWorkflowAs: (chain: Workflow, projectPath?: string) =>
    invokeIpc<C8cApi["saveWorkflowAs"]>("workflows:save-as", chain, projectPath),
  exportWorkflowCopy: (chain: Workflow, projectPath?: string) =>
    invokeIpc<C8cApi["exportWorkflowCopy"]>("workflows:export-copy", chain, projectPath),
  openWorkflowFile: () => invokeIpc<C8cApi["openWorkflowFile"]>("workflows:open-file"),
  createWorkflow: (projectPath: string, name: string, chain: Workflow) =>
    invokeIpc<C8cApi["createWorkflow"]>("workflows:create", projectPath, name, chain),
  ...
  deleteWorkflow: (filePath: string) => invokeIpc<C8cApi["deleteWorkflow"]>("workflows:delete", filePath),
  ...
  runChain: (
    ...
    invokeIpc<C8cApi["runChain"]>("executor:run", chain, input, projectPath, workflowPath, webSearchBackend),
  cancelRun: (runId: string) => invokeIpc<C8cApi["cancelRun"]>("executor:cancel", runId),
  pauseRun: (runId: string) => invokeIpc<C8cApi["pauseRun"]>("run:pause", runId),
  resumeRun: (runId: string) => invokeIpc<C8cApi["resumeRun"]>("run:resume", runId),
  rerunFrom: (
    fromNodeId: string,
    workflow: Workflow,
    workspace: string,
  ...
  listRuns: (projectPath: string) => invokeIpc<C8cApi["listRuns"]>("executor:list-runs", projectPath),
  loadRunResult: (workspace: string) =>
    invokeIpc<C8cApi["loadRunResult"]>("executor:load-run-result", workspace),
  openReport: (reportPath: string) => invokeIpc<C8cApi["openReport"]>("executor:open-report", reportPath),
  getActiveExecutions: () => invokeIpc<C8cApi["getActiveExecutions"]>("executor:get-active-executions"),
  persistArtifactsFromRun: (input: PersistArtifactsFromRunRequest) =>
    invokeIpc<C8cApi["persistArtifactsFromRun"]>("executor:persist-artifacts-from-run", input),
  listProjectArtifacts: (projectPath: string) =>
    invokeIpc<C8cApi["listProjectArtifacts"]>("executor:list-project-artifacts", projectPath),
  listProjectCaseStates: (projectPath: string) =>
    invokeIpc<C8cApi["listProjectCaseStates"]>("executor:list-project-case-states", projectPath),
  loadProjectFactoryBlueprint: (projectPath: string) =>
    invokeIpc<C8cApi["loadProjectFactoryBlueprint"]>("factory:load-blueprint", projectPath),
  saveProjectFactoryBlueprint: (input: SaveProjectFactoryBlueprintInput) =>
    invokeIpc<C8cApi["saveProjectFactoryBlueprint"]>("factory:save-blueprint", input),
  loadProjectFactoryState: (projectPath: string) =>
    invokeIpc<C8cApi["loadProjectFactoryState"]>("factory:load-state", projectPath),
  spawnFactoryCasesFromArtifact: (input: SpawnFactoryCasesFromArtifactInput) =>
    invokeIpc<C8cApi["spawnFactoryCasesFromArtifact"]>("factory:spawn-cases-from-artifact", input),

  // Libraries
  listLibraries: () => invokeIpc<C8cApi["listLibraries"]>("libraries:list"),
  installLibrary: (id: string) => invokeIpc<C8cApi["installLibrary"]>("libraries:install", id),
  removeLibrary: (id: string) => invokeIpc<C8cApi["removeLibrary"]>("libraries:remove", id),
  scanLibraries: () => invokeIpc<C8cApi["scanLibraries"]>("libraries:scan"),
  listMarketplaces: () => invokeIpc<C8cApi["listMarketplaces"]>("plugins:list-marketplaces"),
  ...
  setPluginEnabled: (pluginId: string, enabled: boolean) =>
    invokeIpc<C8cApi["setPluginEnabled"]>("plugins:set-enabled", pluginId, enabled),

  // Templates
  listTemplates: () => invokeIpc<C8cApi["listTemplates"]>("templates:list"),
  listPopularProjectTemplates: (projectPath: string, limit?: number) =>
    invokeIpc<C8cApi["listPopularProjectTemplates"]>("templates:list-popular-project", projectPath, limit),
  recordProjectTemplateUsage: (projectPath: string, templateId: string) =>
    invokeIpc<C8cApi["recordProjectTemplateUsage"]>("templates:record-usage", projectPath, templateId),
  ...
  fetchHubTemplate: (templateId: string) =>
    invokeIpc<C8cApi["fetchHubTemplate"]>("templates:fetch-hub-template", templateId),
  refreshCatalog: () => invokeIpc<C8cApi["refreshCatalog"]>("templates:refresh-catalog"),
  inspectCreateEntryProject: (projectPath: string) =>
    invokeIpc<C8cApi["inspectCreateEntryProject"]>("templates:inspect-project", projectPath),
  routeCreateEntry: (input) =>
    invokeIpc<C8cApi["routeCreateEntry"]>("templates:route-create-entry", input),
  generateWorkflow: (
    description: string,
    availableSkills: Pick<DiscoveredSkill, "name" | "category" | "description">[],
    projectPath?: string,
  ) =>
    invokeIpc<C8cApi["generateWorkflow"]>("templates:generate", description, availableSkills, projectPath),
  cancelGenerate: () => invokeIpc<C8cApi["cancelGenerate"]>("templates:cancel-generate"),

  // System
  getAppVersion: () => invokeIpc<C8cApi["getAppVersion"]>("system:get-app-version"),
  getDesktopRuntime: () => invokeIpc<C8cApi["getDesktopRuntime"]>("system:get-desktop-runtime"),
  ...
  updateDesktopMenuState: (state: DesktopMenuState) =>
    invokeIpc<C8cApi["updateDesktopMenuState"]>("system:update-desktop-menu-state", state),
  ...
  getProjectStatus: (projectPath: string | null) =>
    invokeIpc<C8cApi["getProjectStatus"]>("system:get-project-status", projectPath),
  getClaudeCodeSubscriptionStatus: () =>
    invokeIpc<C8cApi["getClaudeCodeSubscriptionStatus"]>("system:get-claude-subscription-status"),
  getProviderDiagnostics: () =>
    invokeIpc<C8cApi["getProviderDiagnostics"]>("system:get-provider-diagnostics"),
  updateProviderSettings: (patch: Partial<ProviderSettings>) =>
    invokeIpc<C8cApi["updateProviderSettings"]>("system:update-provider-settings", patch),
  setCodexApiKey: (apiKey: string) =>
    invokeIpc<C8cApi["setCodexApiKey"]>("system:set-codex-api-key", apiKey),
  clearCodexApiKey: () => invokeIpc<C8cApi["clearCodexApiKey"]>("system:clear-codex-api-key"),
  ...
  setTelemetryConsent: (enabled: boolean) =>
    invokeIpc<C8cApi["setTelemetryConsent"]>("system:set-telemetry-consent", enabled),
  ...
  // Auto-updater
  checkForUpdate: () => invokeIpc<C8cApi["checkForUpdate"]>("system:check-for-update"),
  installUpdate: () => invokeIpc<C8cApi["installUpdate"]>("system:install-update"),
  getUpdateStatus: () => invokeIpc<C8cApi["getUpdateStatus"]>("system:get-update-status"),
  ...
  chatSendMessage: (workflowPath: string, message: string, projectPath: string, currentWorkflow: Workflow) =>
    invokeIpc<C8cApi["chatSendMessage"]>("chat:send-message", workflowPath, message, projectPath, currentWorkflow),
  ...
  chatGetActiveSession: (workflowPath: string) =>
    invokeIpc<C8cApi["chatGetActiveSession"]>("chat:get-active-session", workflowPath),
  ...
  approveNode: (runId: string, nodeId: string, editedContent?: string) =>
    invokeIpc<C8cApi["approveNode"]>("executor:approve", runId, nodeId, editedContent),
  ...
  overrideEvaluator: (runId: string, nodeId: string) =>
    invokeIpc<C8cApi["overrideEvaluator"]>("executor:override-evaluator", runId, nodeId),
  listHumanTasks: (projectPath?: string) =>
    invokeIpc<C8cApi["listHumanTasks"]>("executor:list-human-tasks", projectPath),
  loadHumanTask: (taskId: string, workspace: string) =>
    invokeIpc<C8cApi["loadHumanTask"]>("executor:load-human-task", taskId, workspace),
  submitHumanTask: (taskId: string, workspace: string, input: HumanTaskSubmitInput) =>
    invokeIpc<C8cApi["submitHumanTask"]>("executor:submit-human-task", taskId, workspace, input),
  rejectHumanTask: (taskId: string, workspace: string, comment?: string, idempotencyKey?: string) =>
    invokeIpc<C8cApi["rejectHumanTask"]>("executor:reject-human-task", taskId, workspace, comment, idempotencyKey),
  ...
  runBatch: (
    ...
    invokeIpc<C8cApi["runBatch"]>("executor:run-batch", workflow, inputs, concurrency, stopOnFailure, projectPath, workflowPath),
  cancelBatch: (batchId: string) => invokeIpc<C8cApi["cancelBatch"]>("executor:cancel-batch", batchId),
  ...
  listProjectFiles: (projectPath: string, query?: string) =>
    invokeIpc<C8cApi["listProjectFiles"]>("files:list-project", projectPath, query),
  readFileContent: (filePath: string, projectPath: string) =>
    invokeIpc<C8cApi["readFileContent"]>("files:read-content", filePath, projectPath),

  // MCP servers
  mcpListServers: (provider: ProviderId, projectPath?: string) =>
    invokeIpc<C8cApi["mcpListServers"]>("mcp:list-servers", provider, projectPath),
  ...
  mcpAddServer: (provider: ProviderId, server: McpServerInfo, projectPath?: string) =>
    invokeIpc<C8cApi["mcpAddServer"]>("mcp:add-server", provider, server, projectPath),
  mcpUpdateServer: (provider: ProviderId, name: string, server: McpServerInfo, projectPath?: string) =>
    invokeIpc<C8cApi["mcpUpdateServer"]>("mcp:update-server", provider, name, server, projectPath),
  mcpRemoveServer: (provider: ProviderId, name: string, scope: McpServerInfo["scope"], projectPath?: string) =>
    invokeIpc<C8cApi["mcpRemoveServer"]>("mcp:remove-server", provider, name, scope, projectPath),
  mcpToggleServer: (provider: ProviderId, name: string, scope: McpServerInfo["scope"], disabled: boolean, projectPath?: string) =>
    invokeIpc<C8cApi["mcpToggleServer"]>("mcp:toggle-server", provider, name, scope, disabled, projectPath),
  mcpTestServer: (provider: ProviderId, name: string, scope: McpServerInfo["scope"], projectPath?: string) =>
    invokeIpc<C8cApi["mcpTestServer"]>("mcp:test-server", provider, name, scope, projectPath),
  mcpDiscoverTools: (provider: ProviderId, serverName?: string, projectPath?: string) =>
    invokeIpc<C8cApi["mcpDiscoverTools"]>("mcp:discover-tools", provider, serverName, projectPath),
  mcpSetPluginServerApproved: (serverId: string, approved: boolean) =>
    invokeIpc<C8cApi["mcpSetPluginServerApproved"]>("mcp:set-plugin-server-approved", serverId, approved),
}
...
if (typeof __TEST_MODE__ !== "undefined" && __TEST_MODE__) {
  const testHarness: C8cTestHarnessApi = {
    getEnvironment: () => invokeIpc<C8cTestHarnessApi["getEnvironment"]>("test-harness:get-environment"),
    seedProjects: (input) => invokeIpc<C8cTestHarnessApi["seedProjects"]>("test-harness:seed-projects", input),
    resetPersistentState: () =>
      invokeIpc<C8cTestHarnessApi["resetPersistentState"]>("test-harness:reset-persistent-state"),
  }
